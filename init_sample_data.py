...
def init_sample_vocabulary():
    ...
    vocab_items = [
        # Spanish vocabulary
        (user_map.get('user_002', 1), 'hola', 'hello', 'es', 3, datetime.utcnow() + timedelta(days=1)),
        (user_map.get('user_002', 1), 'gracias', 'thank you', 'es', 5, datetime.utcnow() + timedelta(days=2)),
        (user_map.get('user_002', 1), 'comida', 'food', 'es', 2, datetime.utcnow()),
        
        # Chinese vocabulary
        (user_map.get('user_003', 1), '你好', 'hello', 'zh', 4, datetime.utcnow() + timedelta(days=1)),
        (user_map.get('user_003', 1), '谢谢', 'thank you', 'zh', 3, datetime.utcnow() + timedelta(hours=12)),
        (user_map.get('user_003', 1), '一', 'one', 'zh', 6, datetime.utcnow() + timedelta(days=3)),
        
        # French vocabulary
        (user_map.get('user_001', 1), 'bonjour', 'hello', 'fr', 4, datetime.utcnow() + timedelta(days=1)),
        (user_map.get('user_001', 1), 'restaurant', 'restaurant', 'fr', 2, datetime.utcnow()),
        (user_map.get('user_001', 1), 'menu', 'menu', 'fr', 1, datetime.utcnow() - timedelta(hours=1)),
    ]
...
def verify_sample_data():
    ...
    try:
        ...
        logger.info(f"ChromaDB embeddings: {total_embeddings} records across {len(collections)} collections")
