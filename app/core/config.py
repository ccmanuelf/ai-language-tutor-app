...
class Settings(BaseSettings):
    ...
    # Database settings
    DATABASE_URL: str = Field(default="mysql+pymysql://root:password@localhost/ai_language_tutor", description="MariaDB connection URL")
    CHROMADB_PATH: str = Field(default="./data/chromadb", description="ChromaDB storage path")
    DUCKDB_PATH: str = Field(default="./data/local/app.duckdb", description="DuckDB local storage path")
    ...
    QWEN_API_KEY: Optional[str] = Field(default=None, description="Alibaba Qwen API key")
    IBM_WATSON_STT_API_KEY: Optional[str] = Field(default=None, description="IBM Watson STT API key")
    IBM_WATSON_TTS_API_KEY: Optional[str] = Field(default=None, description="IBM Watson TTS API key")
    IBM_WATSON_STT_URL: Optional[str] = Field(default=None, description="IBM Watson STT service URL")
    IBM_WATSON_TTS_URL: Optional[str] = Field(default=None, description="IBM Watson TTS service URL")
    
    # Ollama settings for local LLMs
    OLLAMA_HOST: str = Field(default="http://localhost:11434", description="Ollama server URL")
    
    # Budget management
    MONTHLY_BUDGET_USD: float = Field(default=30.0, description="Monthly API budget in USD")
    COST_TRACKING_ENABLED: bool = Field(default=True, description="Enable cost tracking")
    
    # Security settings
    JWT_SECRET_KEY: str = Field(default="jwt-secret-key-change-in-production", description="JWT signing key")
