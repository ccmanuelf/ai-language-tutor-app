...
class AuthenticationService:
    ...
    def generate_child_pin(self) -> str:
        ...
        return "".join(
            secrets.choice("0123456789") for _ in range(self.config.CHILD_PIN_LENGTH)
        )
    ...
    def create_refresh_token(self, user_id: str) -> str:
        ...
        try:
            encoded_jwt = jwt.encode(
                token_data, self.config.SECRET_KEY, algorithm=self.config.ALGORITHM
            )
    ...
    def refresh_access_token(self, refresh_token: str) -> Tuple[str, str]:
        ...
        try:
            ...
            if (
                not jti
                or jti not in self.refresh_tokens
                or not self.refresh_tokens[jti]["is_active"]
            ):
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    def get_session(self, session_id: str) -> Optional[SessionData]:
        ...
        if session.last_activity < datetime.now(timezone.utc) - timedelta(
            hours=expire_hours
        ):
    ...
    def authenticate_user(
        self, user_id: str, password: str, user_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def authenticate_child_pin(
        self, user_id: str, pin: str, user_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def cleanup_expired_sessions(self) -> int:
        ...
        for session in list(self.active_sessions.values()):
            if session.last_activity < now - timedelta(
...
class RateLimiter:
    ...
    def is_allowed(self, key: str, max_requests: int, window_seconds: int) -> bool:
        ...
        if key in self.requests:
            self.requests[key] = [
                (ts, count) for ts, count in self.requests[key] if ts > window_start
            ]
        else:
            self.requests[key] = []
...
def check_rate_limit(
    request: Request, max_requests: int = 100, window_seconds: int = 3600
):
