...
class ConversationManager:
    ...
    def __init__(self):
        ...
        self.context_compression_threshold = (
            50  # Compress when exceeding this many messages
        )
    ...
    async def start_conversation(
        ...
        if context.is_scenario_based and scenario_progress_id:
            ...
            if scenario_data:
                system_message = self._create_scenario_system_message(
    ...
    async def send_message(
        ...
        **kwargs,
        ...
        try:
            ...
            # Handle scenario-based interactions
            ...
            if context.is_scenario_based and context.scenario_progress_id:
                try:
                    ...
                    # Update scenario phase in context
            ...
            await self._update_conversation_context(
                conversation_id=conversation_id, insights=learning_insights
            )
            ...
            response_data = {
                ...
                "ai_metadata": {
                    ...
                    "provider": ai_response.provider,
                    "cost": ai_response.cost,
                    "is_fallback": ai_response.metadata.get("router_selection", {}).get(
                        "is_fallback", False
                    ),
                },
            }
...
Remember: Be patient, encouraging, and focus on practical language use."""
    ...
    def _create_scenario_system_message(
        self, context: ConversationContext, scenario_data: Dict[str, Any]
    ) -> str:
        """Create system message tailored for scenario-based learning"""
...
- Your Role: You are playing the role of a {context.ai_role if hasattr(context, "ai_role") else "conversation partner"}
- Student's Role: The student is playing the role of a {context.user_role if hasattr(context, "user_role") else "conversation participant"}
...
INSTRUCTIONS:
1. Stay in character as the {context.ai_role if hasattr(context, "ai_role") else "conversation partner"}
...
Remember: Make this feel like a real conversation in the scenario while supporting the student's learning goals."""
    ...
    async def _generate_learning_insights(
        ...
        potential_new_vocab = list(ai_words - recent_vocabulary)[
            :3
        ]  # Limit to 3 new words
    ...
    async def _update_conversation_context(
        self, conversation_id: str, insights: LearningInsight
    ):
    ...
    async def _maybe_compress_context(self, conversation_id: str):
        ...
        if len(messages) > self.context_compression_threshold:
            ...
            compressed_count = (
                len(messages) - len(recent_messages) - len(system_messages)
            )
            ...
            if compressed_count > 0:
                ...
                logger.info(
                    f"Compressed conversation {conversation_id}: {compressed_count} messages summarized"
                )
    ...
    async def _save_conversation_to_db(
        ...
        logger.info(
            f"Saving conversation {conversation_id} to database with status: {status}"
        )
