...
class TestZZZImportErrorHandling:
    """Test import error handling for openai library (used by Qwen)
    ...
    def test_import_error_handling(self):
        ...
        original_qwen_service = sys.modules.get("app.services.qwen_service")
        ...
        try:
            ...
            # Remove the qwen_service module to force reimport
            if "app.services.qwen_service" in sys.modules:
                del sys.modules["app.services.qwen_service"]

            # Now import the module - this should trigger the except block
            import app.services.qwen_service as qwen_module

            # Verify that the ImportError was caught and handled
            assert qwen_module.QWEN_AVAILABLE is False
            assert qwen_module.openai is None
            ...
            # Restore the original qwen_service module
            if original_qwen_service is not None:
                sys.modules["app.services.qwen_service"] = original_qwen_service
            elif "app.services.qwen_service" in sys.modules:
...
class TestQwenServiceInitialization:
    ...
    def test_service_initialization_with_deepseek_key(self):
        ...
        with patch("app.services.qwen_service.get_settings") as mock_settings:
            mock_settings.return_value = Mock(
                DEEPSEEK_API_KEY="test-deepseek-key", QWEN_API_KEY=None
            )
    ...
    def test_service_initialization_with_qwen_key(self):
        ...
        with patch("app.services.qwen_service.get_settings") as mock_settings:
            mock_settings.return_value = Mock(
                DEEPSEEK_API_KEY=None, QWEN_API_KEY="test-qwen-key"
            )
            with patch("app.services.qwen_service.QWEN_AVAILABLE", True):
                with patch("app.services.qwen_service.openai") as mock_openai:
                    mock_client = Mock()
    ...
    def test_service_initialization_without_api_key(self):
        ...
        with patch("app.services.qwen_service.get_settings") as mock_settings:
            mock_settings.return_value = Mock(DEEPSEEK_API_KEY=None, QWEN_API_KEY=None)
            with patch("app.services.qwen_service.QWEN_AVAILABLE", True):
                service = QwenService()
...
class TestHelperMethods:
    ...
    def test_select_model_with_deepseek(self):
        ...
        with patch("app.services.qwen_service.get_settings") as mock_settings:
            mock_settings.return_value = Mock(
                DEEPSEEK_API_KEY="test-key", QWEN_API_KEY=None
            )
            service = QwenService()
    ...
    def test_select_model_with_qwen(self):
        ...
        with patch("app.services.qwen_service.get_settings") as mock_settings:
            mock_settings.return_value = Mock(
                DEEPSEEK_API_KEY=None, QWEN_API_KEY="test-key"
            )
            service = QwenService()
    ...
    def test_call_qwen_api_with_params(self):
        ...
        result = service._call_qwen_api(
            "qwen-plus", "Test prompt", {"max_tokens": 500, "temperature": 0.9}
        )
...
class TestResponseBuilding:
    ...
    def test_build_success_response(self):
        ...
        result = service._build_success_response(
            mock_response, "qwen-plus", "zh", 1.5, {"user_id": "test123"}
        )
    ...
    def test_build_success_response_no_context(self):
        ...
        result = service._build_success_response(
            mock_response, "qwen-plus", "zh", 1.5, None
        )
...
class TestGenerateResponse:
    ...
    async def test_generate_response_success(self):
        ...
        result = await service.generate_response(message="你好", language="zh")
    ...
    async def test_generate_response_api_error(self):
        ...
        result = await service.generate_response(message="Hello", language="zh")
    ...
    async def test_generate_response_with_context(self):
        ...
        result = await service.generate_response(
            message="Hello", language="zh", context={"user_id": "user123"}
        )
