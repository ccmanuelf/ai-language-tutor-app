"""
Tests for spaced_repetition_manager_refactored.py
Comprehensive test coverage for facade pattern implementation (refactored version)
"""
...
class TestSpacedRepetitionManagerInit:
    ...
    @patch("app.services.spaced_repetition_manager_refactored.get_db_manager")
    @patch("app.services.spaced_repetition_manager_refactored.SM2Algorithm")
    @patch("app.services.spaced_repetition_manager_refactored.SessionManager")
    @patch("app.services.spaced_repetition_manager_refactored.GamificationEngine")
    @patch("app.services.spaced_repetition_manager_refactored.AnalyticsEngine")
...
class TestAddLearningItem:
    ...
    def test_add_learning_item_delegates_to_algorithm(
        ...
        # Act - Note: parameter order differs from original (item_type is 3rd param)
        ...
        mock_algorithm.add_learning_item.assert_called_once_with(
            1, "es", "vocabulary", "hola", "hello", extra_field="test"
        )
...
class TestReviewItem:
    ...
    @patch("app.services.spaced_repetition_manager_refactored.GamificationEngine")
    @patch("app.services.spaced_repetition_manager_refactored.AnalyticsEngine")
    @patch("app.services.spaced_repetition_manager_refactored.logger")
...
class TestGetDueItems:
    ...
    def test_get_due_items_delegates_to_algorithm(
        ...
        """Test that get_due_items delegates to SM2Algorithm with item_type"""
        # Arrange
        mock_db_manager = Mock()
        mock_get_db_manager.return_value = mock_db_manager
        ...
        # Refactored version passes item_type parameter through
        mock_algorithm.get_due_items.assert_called_once_with(1, "es", 10, "vocabulary")
...
class TestSingletonPattern:
    ...
    def test_get_spaced_repetition_manager_returns_singleton(self):
        ...
        with patch("app.services.spaced_repetition_manager_refactored.get_db_manager"):
            with patch(
                "app.services.spaced_repetition_manager_refactored.SM2Algorithm"
            ):
                with patch(
                    "app.services.spaced_repetition_manager_refactored.SessionManager"
                ):
                    with patch(
                        "app.services.spaced_repetition_manager_refactored.GamificationEngine"
                    ):
                        with patch(
                            "app.services.spaced_repetition_manager_refactored.AnalyticsEngine"
                        ):
                            # Act
