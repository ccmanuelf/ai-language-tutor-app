...
async def test_ai_router_functionality():
    ...
    try:
        ...
        print(f"✅ Test 1: AI router selected provider: {selection.provider_name} with model: {selection.model_name}")
        tests_passed += 1
    ...
    try:
        ...
        print(f"✅ Test 2: Generated response from {response.provider}: '{response.content[:50]}...'")
        tests_passed += 1
    except Exception as e:
    ...
    try:
        ...
        print(f"✅ Test 3: Budget status - Spend: ${status.current_spend:.2f}, Remaining: ${status.remaining_budget:.2f}")
        tests_passed += 1
    except Exception as e:
...
async def test_conversation_functionality():
    ...
    try:
        if conversation_id:
            ...
            print(f"✅ Test 3: Retrieved conversation history with {len(history)} messages")
            tests_passed += 1
        else:
    ...
    try:
        if conversation_id:
            insights = await conversation_manager.generate_learning_insights(conversation_id)
...
async def test_speech_pipeline_functionality():
    ...
    try:
        ...
        print(f"   Audio duration: {getattr(result, 'duration_seconds', 'unknown')} seconds")
        tests_passed += 1
    except Exception as e:
...
async def test_integration_end_to_end():
    ...
    try:
        ...
        print(f"   Conversation: {len(summary.get('session_stats', {}).get('total_messages', 0))} messages")
        tests_passed += 1
    except Exception as e:
